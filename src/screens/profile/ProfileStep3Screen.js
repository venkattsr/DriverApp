...
export default function ProfileStep3Screen({ navigation }) {
  ...
  const openCamera = async (side) => {
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) uploadImage(result.assets[0].uri, side);
    } catch (e) {
  ...
  const uploadImage = async (uri, side) => {
    ...
    const setImage = side === 'front' ? setAadhaarFront : setAadhaarBack;
    setLoading(true);
    Toast.show({ type: 'info', text1: 'Uploading...' });
    try {
      const url = await uploadDriverDocument(user.uid, uri, `aadhaar_${side}`);
      setImage(url);
      Toast.show({ type: 'success', text1: `Aadhaar ${side} uploaded! ✓` });
    } catch (e) {
  ...
  const handleSkip = () => navigation.navigate('ProfileStep4');
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.subtitle}>Upload clear photos for identity verification</Text>

        <UploadBox
          label="Front Side"
          subtitle="Name, DOB, gender and Aadhaar number"
          value={aadhaarFront}
        ...
        <View style={styles.privacyNote}>
          ...
          <Text style={styles.privacyText}>Your Aadhaar is securely stored and encrypted.</Text>
        </View>

        <TouchableOpacity style={[styles.nextBtn, saving && { opacity: 0.7 }]} onPress={handleNext} disabled={saving}>
...
const styles = StyleSheet.create({
  ...
  uploadBtn: {
    ...
    backgroundColor: COLORS.surface, borderRadius: 14, borderWidth: 2, borderColor: COLORS.border,
  ...
  uploadedBox: { borderRadius: 14, overflow: 'hidden', borderWidth: 2, borderColor: COLORS.primary, height: 160, position: 'relative' },
  uploadedImage: { width: '100%', height: '100%' },
  uploadedOverlay: { position: 'absolute', top: 0, left: 0, right: 0, backgroundColor: 'rgba(79, 70, 229, 0.7)', paddingVertical: 6, alignItems: 'center' },
  uploadedText: { color: COLORS.white, fontWeight: '700', fontSize: 13 },
  changeButtons: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.6)' },
  changeBtn: { flex: 1, alignItems: 'center', paddingVertical: 8 },
  changeBtnText: { color: COLORS.white, fontSize: 13, fontWeight: '600' },
  privacyNote: { flexDirection: 'row', backgroundColor: 'rgba(79, 70, 229, 0.1)', borderRadius: 12, padding: 16, marginBottom: 24, borderWidth: 1, borderColor: 'rgba(79, 70, 229, 0.2)', alignItems: 'flex-start' },
  privacyIcon: { fontSize: 20, marginRight: 12 },
  privacyText: { flex: 1, color: COLORS.textSecondary, fontSize: 13, lineHeight: 20 },
  nextBtn: { backgroundColor: COLORS.primary, borderRadius: 12, paddingVertical: 16, alignItems: 'center', shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 12, elevation: 6 },
